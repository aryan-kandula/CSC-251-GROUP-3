 ...
 *
 * Edit History:
 * [02/16/2026 - 4:22 PM] Aryan Kandula - Project created, base class structure and main menu
 * [02/19/2026 - 6:47 PM] Aryan Kandula - Store module and Animal Sales module with breeder support
 * [02/24/2026 - 5:15 PM] Aryan Kandula - Services & Payments module, reports, seed data for testing
 * [02/27/2026 - 7:03 PM] Aryan Kandula - Farm-themed GUI, custom dialogs, FarmButton, FarmBackground
 * [02/28/2026 - 5:41 PM] Aryan Kandula - File I/O added: loadFromCSV() and saveToCSV()
 */

// Written by: Aryan Kandula - StoreItem represents a product sold in the farm store.
class StoreItem {
    private String name;
...
// Written by: Aryan Kandula - Animal represents a farm or breeder animal available for sale.
class Animal {
    ...
    @Override
    public String toString() {
        return String.format("%-10s %-20s $%-8.2f %-18s %s", type, breed, salePrice, source, sold ? "[SOLD]" : "[Available]");
    }
}

// Written by: Aryan Kandula - ServiceRecord tracks a scheduled service appointment and payment status.
class ServiceRecord {
...
// Written by: Aryan Kandula - FarmTheme centralizes all color and font constants for consistent UI.
class FarmTheme {
    ...
    static final Color[] PALETTE  = {
        ...
        new Color(100, 80, 160), new Color(40, 120, 160),
        new Color(100, 25,  25), new Color(170,100,  30)
    };
...
// Written by: Aryan Kandula - FarmButton is a reusable styled button with gradient and hover effect.
...
// Written by: Aryan Kandula - FarmBackground paints a farm scene (sky, barn, fence, flowers) as a backdrop.
class FarmBackground extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2.setPaint(new GradientPaint(0, 0, new Color(180, 220, 255), 0, h * 0.55f, new Color(215, 238, 255)));
        g2.fillRect(0, 0, w, h);
        ...
        drawCloud(g2, 30, 18); drawCloud(g2, 160, 12); drawCloud(g2, w/2-30, 25);
        g2.setPaint(new GradientPaint(0, (int)(h*0.52f), FarmTheme.GREEN, 0, h, FarmTheme.GREEN_DARK));
    ...
    private void drawCloud(Graphics2D g2, int x, int y) {
        ...
        g2.fillOval(x, y+16, 50, 32); g2.fillOval(x+16, y, 50, 46); g2.fillOval(x+36, y+12, 40, 30);
    }

    private void drawBarn(Graphics2D g2, int x, int y) {
        g2.setPaint(new GradientPaint(x, y+30, FarmTheme.BARN_RED, x+110, y+30, FarmTheme.BARN_DARK));
        g2.fillRect(x, y+35, 110, 80);
        int[] rx = {x-10, x+55, x+120}, ry = {y+38, y, y+38};
        g2.setColor(FarmTheme.BROWN.darker()); g2.fillPolygon(rx, ry, 3);
        g2.setColor(new Color(210,165,100)); g2.setStroke(new BasicStroke(2.5f)); g2.drawPolygon(rx, ry, 3);
        g2.setColor(FarmTheme.BROWN); g2.fillRect(x+38, y+75, 34, 40);
        g2.setColor(FarmTheme.HAY_GOLD); g2.fillOval(x+52, y+93, 7, 7);
        g2.setColor(FarmTheme.SKY); g2.fillRect(x+12, y+50, 22, 18);
        g2.setColor(FarmTheme.BROWN.darker()); g2.setStroke(new BasicStroke(2f));
        g2.drawRect(x+12, y+50, 22, 18);
        g2.drawLine(x+23, y+50, x+23, y+68); g2.drawLine(x+12, y+59, x+34, y+59);
    }

    private void drawFence(Graphics2D g2, int w, int y) {
        g2.setColor(new Color(240, 230, 200)); g2.setStroke(new BasicStroke(3f));
        g2.drawLine(0, y, w, y); g2.drawLine(0, y+18, w, y+18);
    ...
    private void drawFlowers(Graphics2D g2, int w, int h) {
        ...
        for (int i = 0; i < xs.length; i++) {
            g2.setColor(FarmTheme.GREEN_DARK); g2.setStroke(new BasicStroke(2f));
            g2.drawLine(xs[i], ys[i], xs[i], ys[i]+20);
            g2.setColor(petals[i % petals.length]);
            for (int p = 0; p < 5; p++) {
                double a = p * (2 * Math.PI / 5);
                g2.fillOval(xs[i]+(int)(7*Math.cos(a))-4, ys[i]+(int)(7*Math.sin(a))-4, 8, 8);
            }
            g2.setColor(FarmTheme.HAY_GOLD); g2.fillOval(xs[i]-4, ys[i]-4, 8, 8);
        }
    }
}

// Written by: Aryan Kandula - FarmDialog provides themed message, option, input, and confirm dialogs.
class FarmDialog {
    ...
    static void message(String title, String body) {
        ...
        ta.setFont(FarmTheme.MONO); ta.setEditable(false);
        ta.setBackground(new Color(255, 252, 232)); ta.setForeground(FarmTheme.TEXT);
        ta.setBorder(new EmptyBorder(10, 14, 6, 14));
        JScrollPane sp = new JScrollPane(ta);
        sp.setBorder(BorderFactory.createCompoundBorder(
            new EmptyBorder(8,14,4,14), new LineBorder(FarmTheme.BROWN, 1, true)));
    ...
    static int option(String title, String prompt, String[] options) {
        ...
        lbl.setFont(FarmTheme.BODY); lbl.setForeground(FarmTheme.TEXT);
        lbl.setBorder(new EmptyBorder(10, 0, 4, 0));
        p.add(lbl, BorderLayout.NORTH);
        JPanel grid = new JPanel(new GridLayout(0, 1, 0, 8));
        grid.setOpaque(false); grid.setBorder(new EmptyBorder(4, 28, 14, 28));
        ...
        d.add(p); d.pack(); d.setLocationRelativeTo(null); d.setVisible(true);
        return result[0];
    }
    ...
    static String input(String title, String prompt) {
        ...
        field.setFont(FarmTheme.BODY); field.setBackground(FarmTheme.CREAM); field.setForeground(FarmTheme.TEXT);
        field.setBorder(BorderFactory.createCompoundBorder(
            new LineBorder(FarmTheme.BROWN, 2, true), new EmptyBorder(5, 8, 5, 8)));
        JPanel mid = new JPanel(new FlowLayout(FlowLayout.CENTER, 0, 6));
        mid.setOpaque(false); mid.add(field);
    ...
    private static JDialog makeDialog(String title, int w, int h) {
        ...
        d.setTitle(title); d.setSize(w, h); d.setModal(true);
        d.setResizable(false); d.setLocationRelativeTo(null);
    ...
    private static JPanel makePanel(String title) {
        JPanel panel = new JPanel(new BorderLayout()) {
            @Override protected void paintComponent(Graphics g) {
                super.paintComponent(g);
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setPaint(new GradientPaint(0, 0, FarmTheme.CREAM, 0, getHeight(), new Color(245, 232, 195)));
                g2.fillRect(0, 0, getWidth(), getHeight()); g2.dispose();
            }
        };
        panel.setBorder(new CompoundBorder(new LineBorder(FarmTheme.BROWN, 3), new EmptyBorder(0, 0, 6, 0)));
        JPanel header = new JPanel(new FlowLayout(FlowLayout.CENTER, 8, 10)) {
            @Override protected void paintComponent(Graphics g) {
                super.paintComponent(g);
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setPaint(new GradientPaint(0, 0, FarmTheme.BARN_RED, getWidth(), 0, FarmTheme.BARN_DARK));
                g2.fillRect(0, 0, getWidth(), getHeight()); g2.dispose();
            }
        };
        header.setOpaque(false);
        JLabel lbl = new JLabel("  " + title);
        lbl.setFont(FarmTheme.HEADER); lbl.setForeground(Color.WHITE);
        header.add(lbl);
    ...
    private static FarmButton btn(String text, Color color, int width) {
        ...
        b.setPreferredSize(new Dimension(width, 40)); return b;
    }
}
...
public class JavaCode_GroupProject_02_POLISHED {
    ...
    // Written by: Aryan Kandula - Reads all inventory, animal, and service data from farm_data.csv.
    static void loadFromCSV() {
        File file = new File(CSV_FILE);
        if (!file.exists()) {
            FarmDialog.message("File Not Found",
                "farm_data.csv was not found.\nPlease ensure it is in the same folder as the program.");
            return;
        }
    ...
    static void showWelcome() {
        ...
        splash.setUndecorated(true); splash.setSize(460, 310);
        splash.setLocationRelativeTo(null); splash.setModal(true);
        ...
        JPanel card = new JPanel(new GridBagLayout()) {
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.fillRoundRect(0, 0, getWidth(), getHeight(), 22, 22); g2.dispose();
            }
        };
        card.setOpaque(false); card.setBorder(new EmptyBorder(16, 30, 16, 30));
        ...
        splash.add(bg); splash.setVisible(true);
    }
    ...
    // Written by: Aryan Kandula - Main navigation loop that drives the entire application.
    static void runMainLoop() {
        String[] menu = {"Store / Inventory", "Animal Sales", "Services & Payments",
                         "View Reports", "Save Data to File", "Exit"};
        while (true) {
    ...
    // Written by: Aryan Kandula - Store module: view, add, sell, and restock inventory items.
    ...
    static void addItem() {
        ...
        if (qs == null) return;
        try {
            double price = Double.parseDouble(ps.trim());
            int quantity = Integer.parseInt(qs.trim());
            if (price < 0 || quantity < 0) { FarmDialog.message("Error","Price and quantity must be 0 or greater."); return; }
            inventory.add(new StoreItem(name.trim(), price, quantity));
    ...
    static void restockItem() {
        ...
        if (qs == null) return;
        try {
            int amount = Integer.parseInt(qs.trim());
            if (amount <= 0) { FarmDialog.message("Error","Restock amount must be a positive number."); return; }
    ...
    // Written by: Aryan Kandula - Animal module: add farm/breeder animals and process sales.
    ...
    static void addAnimal() {
        ...
        try {
            double price = Double.parseDouble(ps.trim());
            if (price < 0) { FarmDialog.message("Error","Sale price must be 0 or greater."); return; }
            animals.add(new Animal(types[ti], breed.trim(), price, source));
    ...
    // Written by: Aryan Kandula - Services module: schedule appointments and track payments.
    ...
    static void viewServices(boolean unpaidOnly) {
        StringBuilder sb = new StringBuilder();
        sb.append(String.format("%-18s %-10s %-18s %-9s %-12s %s%n","Customer","Animal","Service","Fee","Date","Status"));
    ...
    static void scheduleService() {
        ...
        try {
            double fee = Double.parseDouble(fs.trim());
            if (fee < 0) { FarmDialog.message("Error","Fee must be 0 or greater."); return; }
            services.add(new ServiceRecord(customer.trim(), animalType.trim(), svcTypes[si], fee, date.trim()));
    ...
    static void markPaid() {
        ...
        sr.markPaid(); revenue += sr.getFee();
        FarmDialog.message("Payment Recorded",
            "Payment of $" + String.format("%.2f",sr.getFee()) + " recorded for " + sr.getCustomerName() + ".");
    }

    // Written by: Aryan Kandula - Generates a business summary report across all modules.
